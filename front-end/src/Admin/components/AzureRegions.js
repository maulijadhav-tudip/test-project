...
class AzureRegions extends Component {
  static propTypes = {
  ...
  loadRegions() {
    ...
    fetch("/api/azure/regions", {
      ...
      .then(json => {
        let regions = json;
        let index = regions.findIndex(x => x.name === this.props.region.name);
        if (index === -1) index = 0;
        this.setState({
          regions: regions,
  ...
  render() {
    ...
    for (var j = 0; j < this.state.regions.length; j++) {
      options.push({
        value: j,
        label: this.state.regions[j]["display_name"]
      });
    }
